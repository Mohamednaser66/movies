...
static  List<MovieEntity> historyMovies=[];
        ...
        children: [
          InkWell(
            onTap: ()async{
              ...
              if(historyMovies.length==10){
                historyMovies.removeAt(0);
              }
              Navigator.pushNamed(context, RoutesManger.movieDetails,arguments: movie);
              SharedPreferences preferences =await SharedPreferences.getInstance();
              List<Map<String,dynamic>> historyToJson=historyMovies.map((e) => e.toJson(),).toList();
              final String json =jsonEncode(historyToJson);
              preferences.setString(CacheConstant.history, json);
